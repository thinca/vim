 ...
 *                           otherwise
 *   *name_end             - points to the '(' of the parameter list, or to
 *                           the '<' when generics were found
 *
    ...
    char_u	**name_end)
{
    ...
    if (*p == '<')
    {
	// Generic function.  The caller rejects it, so do not parse the type
	// arguments here.
	*gen_bracket_start = p;
	*name_end = p;
	return OK;
...
compile_nested_function(exarg_T *eap, cctx_T *cctx, garray_T *lines_to_free)
{
	...
	if (!ends_excmd2(name_start, name_end))
	{
		...
		{
		    // A generic function needs type arguments at every call,
		    // so it cannot be reduced to the funcref that the
		    // dictionary entry holds.
		    char_u  *lval = vim_strnsave(name_start,
						    bracket_start - name_start);

		    emsg_funcname(e_generic_func_missing_type_args_str,
					   lval == NULL ? name_start : lval);
		    vim_free(lval);
		    return NULL;
		}
	    }
